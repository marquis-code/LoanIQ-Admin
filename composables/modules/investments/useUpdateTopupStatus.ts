...
export const useUpdateTopupStatus = () => {
    ...
    const updateTopupStatus = async (actionId: string, status: any) => {
      loading.value = true;
      try {
        const res = await $_approve__reject_topup_investment(actionId, { status: status.status === 'completed' ? 'approved' : 'rejected' }) as any
        if (res.type !== 'ERROR') {
