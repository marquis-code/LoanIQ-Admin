<template>
  <section>
   ...
   <section v-if="canView('roles-and-permission')">
    ...
    <CoreDrawer v-if="canView('roles-and-permission')" :title="selectedRole && showPermissionsModal ? `Edit (${selectedRole?.name}) Permissions` : `View (${selectedRole?.name}) Permissions`" :description="`Below are the permissions attached to the ${selectedRole?.name} role`"  :showFooter="false" :show="!!selectedRole && showPermissionsModal" @close="closePermissionsModal">
      <template #content>
        <ModulesPermissions
            @close="closePermissionsModal"
            :role="selectedRole"
  ...
  function saveRole(newRole: Role) {
    if (selectedRole.value) {
      ...
      const index = rolesList.findIndex((r) => r.name === selectedRole.value.name);
      if (index !== -1) rolesList[index] = newRole;
    } else {
      // Add new role
